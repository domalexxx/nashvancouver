...
if( check_post_expiration( $postID, "event" ) <= 0 ) {
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              ...
              <?php
				 ...
				 if(have_posts())
				 {
					  while(have_posts())
					  {
					   ...
					   $link = get_post_meta($eventId,"link",true);
					   
					   $checkPayment = $wpdb->get_row("select * from $payHistorytab where postId = '$eventId' and postType='event' ORDER BY tid DESC limit 0,1");
						$numDays = 0;
              ...
              <div id="classified">
                <div class="form-group event-name">
				  ...
				  <div class="bg-expire-2">Размещение вашего мероприятия заканчивается через <?php echo ceil($numDays); ?> дня
                    ...
                    <div class="form-group">
                      <div class="website text-left col-xs-10 col-sm-10 col-md-10 col-lg-12 control-label" for="">Вебсайт: <span>
                        <?php
							...
							if (empty($parsed['scheme'])){ $urlStr = 'http://' . ltrim($website, '/'); }
							else { $urlStr = $website; }*/
							echo '<a target="_blank" href="'.addhttp($website).'">'.mb_strimwidth(addhttp($website),0,25,'...').'</a>';	
						?>
                        </span> </div>
                    </div>
                    <?php } ?>
                    ...
                    <?php if(!empty($evPrice)) { ?>
						<div class="form-group">
						  <div class="price text-left col-xs-10 col-sm-10 col-md-10 col-lg-12 control-label">Стоимость: <span class="event-price">
								...
								<?php if(!empty($link)) { ?>
									<a href="<?php echo addhttp($link); ?>"><button class='btn btn-primary buy-ticket-event pull-right'>Kупить билеты</button></a>
								<?php } ?>
						   </div>
						</div>
                    <?php } ?>
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 single-business-form">
						<form role="form" name="contact-us-form" id="contact-us-form" method="post">
							...
							<div class="text-center form-group">
								<button type="submit" class="btn btn-primary" value="Отправить сообщение владельцу бизнеса" name="sendmessage" id="sendmessage">Отправить сообщение владельцу мероприятия</button>
							</div>
						</form>
					</div>
                ...
                <div class="row event">
                  <?php
								...
								wp_reset_query();
								  ...
								  query_posts( array('post_type'=>'event', 'post__not_in' => array( $postID ), 'post_status'=>'publish', 'posts_per_page'=>'3', 'post__in' => $my_post_array,
								          'tax_query' => array(
											array(
												'taxonomy' => 'event_cat',
												'field'    => 'id',
			...
			<form name="deleteform" id="deleteform" method="post">
				<div class="modal-content text-center">
					<div class="modal-header">
						...
						<h4 class="title4">Ваше мероприятие будет удалено!</h4>
					</div>
